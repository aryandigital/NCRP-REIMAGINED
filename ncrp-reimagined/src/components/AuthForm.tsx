...
export default function AuthForm({ mode }: { mode: "signin" | "signup" }) {
  ...
  return <form onSubmit={submit} className="panel p-6 sm:p-8" noValidate aria-busy={pending}>
    ...
    {!creating && <button type="button" onClick={() => { setEmail("judge@raksha.demo"); setPassword("Raksha-2026"); }} className="mt-4 w-full rounded-[8px] border border-dashed border-service/40 bg-service/5 px-3 py-2 text-left text-xs leading-5 text-ink-soft hover:bg-service/10"><span className="font-bold text-service">Reviewer access:</span> judge@raksha.demo / Raksha-2026 — tap to fill.</button>}
