...
/**
 * Partial translation for static atlas copy only. Homepage and shared chrome
 * use React dictionaries. Operational routes must never be DOM-translated.
 */
...
function translateText(node: Text, dictionary: Record<string, string> | null) {
  ...
  // A React update replaces the source, not just our translated output.
  const original = cached && node.data === cached.rendered ? cached.source : node.data;
  const leading = original.match(/^\s*/)?.[0] ?? "";
  const trailing = original.match(/\s*$/)?.[0] ?? "";
  const core = original.trim();
  const translated = dictionary?.[core];
  const next = dictionary && translated ? `${leading}${translated}${trailing}` : original;
  originals.set(node, { source: original, rendered: next });
...
function translateTree(root: Node, dictionary: Record<string, string> | null) {
  const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, {
    acceptNode(node) {
      ...
      if (parent.closest("[data-raksha-i18n='react'], [translate='no'], [contenteditable], input, textarea, select")) return NodeFilter.FILTER_REJECT;
...
export default function SiteLanguageLayer() {
  ...
  useEffect(() => {
    ...
    if (pathname !== "/atlas" && !pathname.startsWith("/atlas/")) return;
    ...
    const apply = () => {
      timer = undefined;
      if (disposed || window.location.pathname !== pathname || !root.isConnected) return;
      observer.disconnect();
      translateTree(root, dictionary);
      // English only restores translations once; it never watches React updates.
      if (dictionary) observer.observe(root, { childList: true, subtree: true, characterData: true });
    };
